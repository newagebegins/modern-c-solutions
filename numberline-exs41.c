...
// Exs 41. Improve the main of the example such that it is able to
// cope with arbitrarily long input lines.
int main(void) {
  size_t tsize = 256; // total size
  char* lbuf = malloc(tsize);
  for (;;) {
    size_t wsize = tsize; // working size
    char* wbuf = lbuf;
    while (!fgetline(wsize, wbuf, stdin)) {
      if (!lbuf[0]) return 0;
      assert(!lbuf[tsize-1]); // should be a string
      wsize = tsize+1; // + \0 pos
      tsize *= 2;
      lbuf = realloc(lbuf, tsize);
      wbuf = lbuf + wsize - 2; // Start at \0
    }
    size_t n;
